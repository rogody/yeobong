...
RAG data ingestion
1. (자동) 메인 앱 실행 시 위 메모리 모듈이 알아서 conversations/messages/message_chunks/chunk_embeddings/message_embeddings 테이블을 채웁니다.
2. (수동) 별도 스크립트에서 MemoryStore.create_conversation / upsert_participant / add_participant_to_conversation 등을 호출해 메타데이터를 구성할 수 있습니다.
3. MemoryStore.save_with_embedding(conv_id, role, content, author_id)만 호출하면 메시지 원문, 청크, 임베딩이 모두 저장됩니다.
